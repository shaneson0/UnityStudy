...
static public class NGUIText
{
	...
	static public void Update (bool request)
	{
        try {
           ...
           if(Application.isEditor) throw (e);
        }
	}
		...
		ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike, ref bool ignoreColor)
	{
        ...
        if (text[index + 1] == 't' && text[index + 2] == 'a' && text[index + 3] == 'g' && text[index + 4] == '=') {
            int closingBracket = text.IndexOf(']', index + 4);
	...
	static public int GetExactCharacterIndex (BetterList<Vector3> verts, BetterList<int> indices, Vector2 pos)
	{
		for (int i = 0; i < indices.size; ++i)
		{
		    ...
		    {
		        UnityEngine.Debug.Log("indices.size: " + indices.size + " i0: " + i0);
		    }
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol == null)
			{
				...
				if (gw != 0f)
				{
                    ...
                    if (Mathf.RoundToInt(x + gw) > regionWidth)
					{
						if (x == 0f) return;
            ...
            if (symbol == null) {
                ...
                if (w != 0f) {
                    ...
                    if (Mathf.RoundToInt(x + w) > rectWidth) {
                        ...
                        x = w;
                        y += finalLineHeight;
                        verts[verts.size - 1] = new Vector3(0, -y - halfSize);
                    } else x += w + finalSpacingX;

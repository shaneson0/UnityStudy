...
//#endif
	...
	void CreateMaterial ()
	{
		...
		else if (mClipCount != 0)
		{
			...
			if (shader == null) shader = FuncUtil.FindShader(shaderName + " " + mClipCount);
		...
		else shader = FuncUtil.FindShader(shaderName);

		// Always fallback to the default shader
		if (shader == null) shader = FuncUtil.FindShader("Unlit/Transparent Colored");
	...
	public void UpdateGeometry (int widgetCount)
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			else
			{
				...
				Debug.LogError(string.Format("Too many vertices on one panel[{0}](parent:{1}):{2}", panel.name, panel.transform.parent.name, verts.size), panel);
	...
	{
#if SHOW_HIDDEN_OBJECTS && UNITY_EDITOR &&!DLL_TYPE

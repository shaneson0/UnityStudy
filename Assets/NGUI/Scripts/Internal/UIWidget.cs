...
public class UIWidget : UIRect
{
    ...
    [System.NonSerialized] public int clearTime;//记录当前需要移除的时间（隐藏的时候）
    ...
    public static Action<bool> setHandlesFun;
    public static Func<bool> getHandlesFun;
    public static Action<UIWidget> drawGizmosFun;
    public static Func<bool> showHandlesFun;
    public static int mHandles = -1;
#if UNITY_EDITOR &&!DLL_TYPE
    ...
    static public bool showHandlesWithMoveTool {
        get {
            if (getHandlesFun != null) {
                return getHandlesFun();
            }
            return (mHandles == 1);
        }
        set {
            if (setHandlesFun == null) {
                return;
            }
            int val = value ? 1 : 0;
    ...
    static public bool showHandles {
        get {
            if (showHandlesFun!=null) {
                return showHandlesFun();
            }
            return false;
        }
    }
	...
	public bool UpdateTransform (int frame)
	{
...
#endif
		{
            ...
            mOldV0 = panel.cachedTransform.worldToLocalMatrix.MultiplyPoint3x4(trans.TransformPoint(x0, y0, 0f));
			mOldV1 = panel.cachedTransform.worldToLocalMatrix.MultiplyPoint3x4(trans.TransformPoint(x1, y1, 0f));
		}
		else if (!panel.widgetsAreStatic && trans.hasChanged)
		{
            ...
            Vector3 v0 = panel.cachedTransform.worldToLocalMatrix.MultiplyPoint3x4(trans.TransformPoint(x0, y0, 0f));
            Vector3 v1 = panel.cachedTransform.worldToLocalMatrix.MultiplyPoint3x4(trans.TransformPoint(x1, y1, 0f));
				...
				Vector3.SqrMagnitude(mOldV1 - v1) > 0.000001f)
			{
                ...
                UIPanel parentPanel =panel.parentPanel;
                while (parentPanel != null) {
                    if (parentPanel.clipping != UIDrawCall.Clipping.None && parentPanel.clipping != UIDrawCall.Clipping.ConstrainButDontClip) {
                    mMoved = true;
                    mOldV0 = v0;
                    mOldV1 = v1;
                    break;
		...
		if (mChanged)
		{
			if (mIsVisibleByAlpha && finalAlpha > 0.001f && shader != null)
			{
				...
				if (geometry.hasVertices)
				{
					...
					{
                        mLocalToPanel = panel.cachedTransform.worldToLocalMatrix * cachedTransform.localToWorldMatrix;
						mMatrixFrame = frame;
					}

...
public class UIInput : MonoBehaviour
{
	...
	protected virtual void Update ()
	{
		...
		if (mCam != null)
		{ 
			if (ignoreEnterKey==false && UICamera.GetKeyDown(mCam.submitKey0))
			{
	...
	protected virtual void Insert (string text)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
            ...
            if (characterLimit > 0 && sb.Length + rl >= characterLimit) { FuncUtil.AddTip(limitMsg); break; };
	...
	protected string GetLeftText ()
	{
        ...
        if (min < 0 || min > mValue.Length) min = 0;

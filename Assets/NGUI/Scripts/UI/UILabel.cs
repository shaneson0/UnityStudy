...
public class UILabel : UIWidget
{
    ...
    private float _textWidh = 0;
    public float textWidh {
        get {
            return Mathf.Min(width, mCalculatedSize.x) * transform.localScale.x;
        }
    }

    public float textHeight {
        get {
            return Mathf.Min(height, mCalculatedSize.y) * transform.localScale.y;
        }
    }

    public float lblHeight {
        get {
            return height * transform.localScale.y;
        }
    }

    public float lblWidth {
        get {
            return width * transform.localScale.x;
        }
    }
    ...
    //解决字体不清晰
    public int defaultFontSize { get { return (trueTypeFont != null) ? Math.Max(mFontSize, (int)dfSize) : (mFont != null ? mFont.defaultSize : 16); } }
	/// <summary>
	...
	static void OnFontChanged (Font font)
	{
		for (int i = 0; i < mList.size; ++i)
		{
			...
			if (lbl != null)
			{
                ...
                FuncUtil.WriteLog("UILabel.OnFontChanged方法错误：Size=" + mList.size + "#buff长度=" + mList.buffer.Length);
            }
		}

		if (mTempPanelList != null)
		{
			for (int i = 0; i < mTempPanelList.Count; ++i)
			{
    ...
    {
        var url = GetUrlAtPosition(UICamera.lastHit.point);
        if (url != null && isHover) 
        {
	...
	{
		if (!isValid) return;
        float scale = mFontSize > dfSize ? 1 : mFontSize / dfSize;
    ...
    public string GetTagAtPosition(Vector3 worldPos) { return GetTagAtCharacterIndex(GetCharacterIndexAtPosition(worldPos, true)); }
    public string GetTagAtPosition(Vector2 localPos) { return GetTagAtCharacterIndex(GetCharacterIndexAtPosition(localPos, true)); }
    public string GetTagAtCharacterIndex(int characterIndex) {
        if (characterIndex != 0 && characterIndex < mText.Length - 6) {
            int linkStart;

            // LastIndexOf() fails if the string happens to begin with the expected text
            if (mText[characterIndex] == '[' &&
                mText[characterIndex + 1] == 't' &&
                mText[characterIndex + 2] == 'a' &&
                mText[characterIndex + 3] == 'g' &&
                mText[characterIndex + 4] == '=') {
                linkStart = characterIndex;
            } else linkStart = mText.LastIndexOf("[tag=", characterIndex);
            ...
            int urlEnd = mText.IndexOf("[/tag]", linkEnd);
            if (urlEnd == -1 || characterIndex <= urlEnd) {
                return mText.Substring(linkStart, linkEnd - linkStart);
            }
        }
    ...
    public static float dfSize = 20.0f;
	...
	public void UpdateNGUIText ()
	{
        ...
        NGUIText.spacingX = mFontSize > dfSize ? mSpacingX : mSpacingX / (mFontSize / dfSize);
        NGUIText.spacingY = mFontSize > dfSize ? mSpacingY : mSpacingY / (mFontSize / dfSize);
		NGUIText.fontScale = isDynamic ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;
    ...
    public Vector3 FindPositionByPos(string strAll, int index) {
        if (index < 0) {
            return Vector3.zero;
        }
        UpdateNGUIText();
        mTempVerts.Clear();
        mTempIndices.Clear();
        NGUIText.PrintCharacterPositions(strAll, mTempVerts, mTempIndices);
        return mTempVerts[mTempIndices.IndexOf(index)];
    }

    public Vector3 FindPositionByStr(string strAll, string strFind, int startIndex = 0) {
        int index = strAll.IndexOf(strFind, startIndex);
        return FindPositionByPos(strAll, index);
    }

    public void UpdateFont() {
        var f = FuncUtil.GetMFont();
        if(f != null){
            trueTypeFont = f;
        }
    }

...
public class UIRoot : MonoBehaviour
{
...
#if UNITY_EDITOR &&!DLL_TYPE || UNITY_IPHONE || UNITY_ANDROID || UNITY_WP8 || UNITY_WP_8_1 || UNITY_BLACKBERRY
    ...
    private  Camera cam;
    private float standardScaleIn = 1;

	protected virtual void Start ()
	{
		MonoBehaviour oc = GetComponentInChildren<UIOrthoCamera>();
        if (oc == null) oc = GetComponentInChildren<UICamera>();
        transform.localScale = new Vector3(standardScaleIn, standardScaleIn, standardScaleIn);
		if (oc != null)
		{
			Debug.LogWarning("UIRoot should not be active at the same time as UIOrthoCamera. Disabling UIOrthoCamera.", oc);
			cam = oc.gameObject.GetComponent<Camera>();
            if (oc is UIOrthoCamera) oc.enabled = false;
            if (cam != null) cam.orthographicSize = (Screen.height / 2) * standardScaleIn + 1f;
		}

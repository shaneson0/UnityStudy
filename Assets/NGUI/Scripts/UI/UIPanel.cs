...
public class UIPanel : UIRect
{
    ...
    private static int MAX_VERT_SIZE = 50000;
	/// <summary>
	...
	void FillAllDrawCalls ()
	{
		...
		for (int i = 0; i < widgets.Count; ++i)
		{
            ...
            if (w.isVisible && w.hasVertices) {
                ...
                if (mat != mt || tex != tx || sdr != sd || (dc.verts.size + w.geometry.verts.size) >= MAX_VERT_SIZE) {
                ...
                if (mat != null || sdr != null || tex != null || (dc.verts.size + w.geometry.verts.size) >= MAX_VERT_SIZE) {
                    if (dc == null) {
                    ...
                    w.drawCall = dc;
                    w.gameObject.SendMessage("UpdateEffDrawCall", dc, SendMessageOptions.DontRequireReceiver);
                ...
                w.drawCall = null;
                w.gameObject.SendMessage("ClearEffDrawCall", null, SendMessageOptions.DontRequireReceiver);
            }
		}
	...
	public UIDrawCall FindDrawCall (UIWidget w)
	{
		...
		for (int i = 0; i < drawCalls.Count; ++i)
		{
            ...
            if (w.hasVertices==false) {
                w.UpdateGeometry(Time.frameCount);
            }
            if (dcStart <= depth && dcEnd >= depth && (dc.verts.size + dc.waitVerts + w.geometry.verts.size) < MAX_VERT_SIZE)
			{
                if (dc.baseMaterial == mat && dc.mainTexture == tex)
				{
					...
					{
						w.drawCall = dc;
                        dc.waitVerts += w.geometry.verts.size;

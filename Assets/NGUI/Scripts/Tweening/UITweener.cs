...
public abstract class UITweener : MonoBehaviour
{
    ...
    //设置同步效果
    public static Dictionary<int, float> synGroupFactorDic = new Dictionary<int, float>();
    public static Dictionary<int, float> synGroupAmountDic = new Dictionary<int, float>();
    private int _synGroup = 0;
    private bool _isInitSynGroup = false;
    ...
    void OnEnable() {
        //设置同步效果,重新启动的时候同步数据
        if (_isInitSynGroup) {
            tweenFactor = synGroupFactorDic[_synGroup];
            amountPerDelta = synGroupAmountDic[_synGroup];
        }
    }
	...
	public void Sample (float factor, bool isFinished)
	{
        ...
        //设置效果同步,记录最后的 tweenFactor 以及 amountPerDelta
        if (_isInitSynGroup && isActiveAndEnabled) {
            synGroupFactorDic[_synGroup] = tweenFactor;
            synGroupAmountDic[_synGroup] = amountPerDelta;
        }
	}
    ...
    //设置效果同步，同样的_synGroup的tween效果会进行同步
    public void SetSynGroup(int index) {
        if (index == 0) return;
        _synGroup = index;
        _isInitSynGroup = true;
        if (synGroupFactorDic.ContainsKey(index) == false) {
            synGroupFactorDic.Add(index, 0);
            synGroupAmountDic.Add(index, 0);
		} 
		OnEnable();
    }
}
